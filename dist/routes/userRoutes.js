...
const authenticateToken = (req, res, next) => {
    ...
    if (!token) {
        return res.status(401).json({ error:true, message: 'No Autorizado. Por favor inicie sesion para acceder' });
    }
    jsonwebtoken_1.default.verify(token, JWT_SECRET, (err, decoded) => {
        if (err) {
            console.error('error en la autenticacion: ', err);
            return res.status(403).json({ error:true, message: 'Token Invalido: No tienes acceso a este recurso' });
        }
        next();
    });
};
